...
TEST(NetworkAdapterInternal, IsValidAllFieldsPresent)
{
    ...
    EXPECT_TRUE(a.isValid())
        << "An adapter with a non-empty name, non-empty IP, and non-zero ID must be valid";
}
...
TEST(NetworkAdapterInternal, IsValidEmptyName)
{
    ...
    EXPECT_FALSE(a.isValid())
        << "An adapter with an empty AdapterName must not be valid";
}
...
TEST(NetworkAdapterInternal, IsValidEmptyIP)
{
    ...
    EXPECT_FALSE(a.isValid())
        << "An adapter with an empty IPv4 address must not be valid";
}
...
TEST(NetworkAdapterInternal, IsValidZeroID)
{
    ...
    EXPECT_FALSE(a.isValid())
        << "An adapter with ID 0 must not be valid — 0 is the unassigned sentinel value";
}
...
TEST(NetworkAdapterInternal, FilterSubnetMaskTrue)
{
    ...
    EXPECT_TRUE(a.filterSubnetMask())
        << "A /24 subnet mask (255.255.255.0) must pass the filter";
}
...
TEST(NetworkAdapterInternal, FilterSubnetMaskFalseClassB)
{
    ...
    EXPECT_FALSE(a.filterSubnetMask())
        << "A /16 subnet mask (255.255.0.0) must not pass the filter — only /24 is accepted";
}
...
TEST(NetworkAdapterInternal, FilterSubnetMaskFalseClassA)
{
    ...
    EXPECT_FALSE(a.filterSubnetMask())
        << "A /8 subnet mask (255.0.0.0) must not pass the filter — only /24 is accepted";
}
...
TEST(NetworkAdapterInternal, ConstructorSetsEligibleWhenSubnetMatches)
{
    ...
    EXPECT_TRUE(a.Eligible)
        << "The constructor must set Eligible=true when filterSubnetMask() returns true (subnet is 255.255.255.0)";
}
...
TEST(NetworkAdapterInternal, ConstructorSetsEligibleFalseWhenSubnetMismatches)
{
    ...
    EXPECT_FALSE(a.Eligible)
        << "The constructor must set Eligible=false when filterSubnetMask() returns false (subnet is not 255.255.255.0)";
}
...
TEST(NetworkAdapterInternal, EqualityByNameAndSubnet)
{
    ...
    EXPECT_EQ(a, b)
        << "Two adapters with the same AdapterName and Subnet must be equal regardless of IP or ID";
}
...
TEST(NetworkAdapterInternal, InequalityDifferentName)
{
    ...
    EXPECT_NE(a, b)
        << "Adapters with different AdapterNames must not be equal";
}
...
TEST(NetworkAdapterInternal, InequalityDifferentSubnet)
{
    ...
    EXPECT_NE(a, b)
        << "Adapters with the same name but different Subnet masks must not be equal";
}
