...
TEST(ThreadBase, IsNotRunningBeforeStart)
{
    ...
    EXPECT_FALSE(t.isRunning())
        << "A freshly constructed thread must not be running until start() is called";
}
...
TEST(ThreadBase, IsRunningAfterStart)
{
    ...
    EXPECT_TRUE(t.isRunning())
        << "isRunning() must return true immediately after start() launches the worker thread";
    t.stop();
}
...
TEST(ThreadBase, IsStoppedAfterStop)
{
    ...
    t.stop();
    EXPECT_FALSE(t.isRunning())
        << "isRunning() must return false after stop() joins the worker thread";
}
...
TEST(ThreadBase, StopWhileStoppedIsNoop)
{
    ...
    EXPECT_NO_THROW(t.stop())
        << "Calling stop() on a thread that was never started must not throw or crash";
    EXPECT_FALSE(t.isRunning())
        << "isRunning() must still be false after a redundant stop() call";
}
...
TEST(ThreadBase, StartWhileRunningIsNoop)
{
    ...
    EXPECT_EQ(countAfter - countBefore, 1)
        << "Calling start() on an already-running thread must not spawn a second worker; "
...
TEST(ThreadBase, TriggerEventWakesThread)
{
    ...
    EXPECT_GT(t.loopCount.load(), before)
        << "triggerEvent() must wake the blocked thread, causing loopCount to increase";
    t.stop();
}
...
TEST(ThreadBase, StopWakesBlockedThread)
{
    ...
    EXPECT_LT(std::chrono::duration_cast<std::chrono::milliseconds>(elapsed).count(), 200)
        << "stop() must unblock and join the thread within one poll cycle (< 200 ms)";
}
...
TEST(ThreadBase, LoopCountDoesNotAdvanceAfterStop)
{
    ...
    EXPECT_EQ(t.loopCount.load(), countAtStop)
        << "After stop(), the worker thread must no longer be running so loopCount must not increase further";
}
